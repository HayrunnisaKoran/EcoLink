...
public class DepartmentsFragment extends Fragment {
    ...
    private void fetchDepartments() {
        RetrofitClient.getApiService().getDepartmentLeaderboard().enqueue(new Callback<List<LeaderboardItem>>() {
            @Override
            ...
            public void onFailure(@NonNull Call<List<LeaderboardItem>> call, @NonNull Throwable t) {
                if (isAdded() && getContext() != null) {
                    Toast.makeText(getContext(), "Bölüm verisi alınamadı", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
